...
$pagination = array( 
		...
		'mid_size' => 2, 
		'prev_next' => true, 
		'prev_text' => '<span class="cmsmasters_theme_icon_slide_' . (!is_rtl() ? 'prev' : 'next') . '">' . (!is_rtl() ? 'prev' : 'next') . '</span>', 
		'next_text' => '<span class="cmsmasters_theme_icon_slide_' . (!is_rtl() ? 'next' : 'prev') . '">' . (!is_rtl() ? 'next' : 'prev') . '</span>', 
		'type' => 'list', 
		'add_args' => false, 
		'add_fragment' => '' 
	);
