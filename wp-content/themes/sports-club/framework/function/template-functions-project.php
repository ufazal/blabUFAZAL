...
function cmsmasters_project_heading($cmsmasters_id, $tag = 'h1', $show = true, $link_redirect = false, $link_url = false) { 
...
function cmsmasters_project_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			$out = '<div class="cmsmasters_project_category clear">' . 
			...
			if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_portfolio_project_cat']) {
				$out .= '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Categories', 'sports-club') . ':' . '</div>' . 
...
function cmsmasters_project_like($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_portfolio_project_like']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Like', 'sports-club') . ':' . '</div>' . 
	...
	if (comments_open()) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_portfolio_project_comment']) {
				$out .= '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Comments', 'sports-club') . ':' . '</div>' . 
					'<div class="project_details_item_desc details_item_desc_comments">' . 
						'<a class="cmsmasters_post_comments cmsmasters_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'sports-club') . ' ' . esc_attr(get_the_title()) . '">' . 
							'<span>' . esc_html(get_comments_number()) . '</span>' . 
						'</a>' . 
					'</div>' . 
				'</div>';
...
function cmsmasters_project_date($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_portfolio_project_date']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Date', 'sports-club') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function cmsmasters_project_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_project_author">' . 
			...
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'sports-club') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($template_type == 'post') {
		$cmsmasters_option = cmsmasters_get_global_options();
		...
		if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_portfolio_project_author']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Author', 'sports-club') . ':' . '</div>' . 
				'<div class="project_details_item_desc vcard author">' . 
					'<span class="cmsmasters_project_author fn" title="' . esc_attr__('Projects by', 'sports-club') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span>' . 
				'</div>' . 
			'</div>';
		}
	}
...
function cmsmasters_project_tags($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_portfolio_project_tag']) {
				$out = '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Tags', 'sports-club') . ':' . '</div>' . 
					'<div class="project_details_item_desc">' . 
...
function cmsmasters_project_link($link_text, $link_url, $link_target, $show = true) {
	...
	if ( 
		...
		$out = '<div class="project_details_item">' . 
			'<div class="project_details_item_title">' . esc_html__('Project Link', 'sports-club') . ':' . '</div>' . 
			'<div class="project_details_item_desc">' . 
				'<span class="cmsmasters_project_tags">' . 
