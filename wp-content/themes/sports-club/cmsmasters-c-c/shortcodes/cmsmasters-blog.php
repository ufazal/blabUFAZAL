...
$more_text = ($more_text != '') ? $more_text : esc_html__('Load More Posts', 'sports-club');


$filter_text = ($filter_text != '') ? $filter_text : esc_html__('Filter', 'sports-club');


$filter_cats_text = ($filter_cats_text != '') ? $filter_cats_text : esc_attr__('All Categories', 'sports-club');
...
";
	...
	if ($filter !== '') {
		$out .= "<div class=\"cmsmasters_post_filter_wrap\">
			<div class=\"cmsmasters_post_filter\">
				<span class=\"cmsmasters_post_filter_loader\"></span>
				<div class=\"cmsmasters_post_filter_block\">
					<a class=\"cmsmasters_post_filter_but cmsmasters_theme_icon_resp_nav button\">
						<span>" . $filter_text . "</span>
					</a>
					<ul class=\"cmsmasters_post_filter_list\">
						<li class=\"current\">
	...
	while ($query->have_posts()) : $query->the_post();
		if ($layout == 'columns') {
			if ($layout_mode == 'puzzle') {
				if (get_post_format() != '') {
					$out .= load_template_part('framework/postType/blog/page/puzzle/' . get_post_format());
				} else {
					$out .= load_template_part('framework/postType/blog/page/puzzle/standard');
				}
			} else {
				if (get_post_format() != '') {
...
if ($pagination !== 'disabled') {
	$out .= '<div class="cmsmasters_wrap_more_posts">';
		...
		if ($pagination == 'pagination' && $query->max_num_pages > 1) {
			...
			$out .= "<div class=\"cmsmasters_wrap_post_loader\">
				<a href=\"javascript:void(0);\" class=\"cmsmasters_button cmsmasters_post_loader\">
					<span>" . $more_text . "</span>
				</a>
			</div>";
		}
