 ...
 */
function thinker_featured_pages() {
	$featured_page_1 = esc_attr( get_theme_mod( 'thinker_featured_page_one_front_page', '0' ) );
	$featured_page_2 = esc_attr( get_theme_mod( 'thinker_featured_page_two_front_page', '0' ) );
	$featured_page_3 = esc_attr( get_theme_mod( 'thinker_featured_page_three_front_page', '0' ) );
...
if ( ! function_exists( 'thinker_view_link' ) ) :
/**
 * Prints an "View" plus off-screen title link for portfolio projects
 */
function thinker_view_link() {
	printf( '<a href="'. esc_url( get_permalink() ) . '" rel="bookmark" class="view-link">' . sprintf( __( 'View <span class="screen-reader-text">%1s</span>', 'thinker' ), esc_attr( strip_tags( get_the_title() ) ) ) . '</a>' );
}
endif; // thinker_view_link