...
/**
* Plugin Name: contact-digital-forecast-Form-with-Captcha
...
$plugin_dir =  ABSPATH . 'wp-content/plugins/contact-digital-forecast/';
if(file_exists($plugin_dir.WPLANG.'.php')) include_once($plugin_dir.WPLANG.'.php');
if (!session_id()) session_start();

function dg_message_remove_malicious($input) {
...
function dg_convertReadable($giefTime) {
	$days = 0; $hrs = 0; $mins = 0;
...
function dg_contact_digital_forecast_form() {
	global $wpdb;
	...
	if(isset($_GET[msg])) {
		$output .= '<p icon="accept">'.dg__e(urldecode($_GET[msg])).'</p>';
	}
	
	//Process form

	if(isset($_POST[dg_submit])) {
		...
		if($_POST['captcha'] != $_SESSION['captcha_code'] OR empty($_POST['captcha'])) {
			$validate = FALSE;
			$errormsg[5] = '<span class="form-error" icon="warning">'.dg__e('The entered letters did not match the control').'</span>';
		}		
		$_SESSION['captcha_code'] = '';
		if(empty($postcontent['contact_fname'])) {
			$validate = FALSE;
			$errormsg[0] = '<span class="form-error" icon="warning">'.dg__e('Please enter your first name').'</span>';		
		}
		if(empty($postcontent['contact_lname'])) {
			$validate = FALSE;
			$errormsg[1] = '<span class="form-error" icon="warning">'.dg__e('Please enter your last name').'</span>';		
		}
		
		if(empty($useremail)) {
			$validate = FALSE;
			if ($postcontent['contact_email'] != $useremail) {
				$errormsg[2] = '<span class="form-error" icon="warning">'.dg__e('Please enter a valid email adress!').'</span>';
			} else {
				$errormsg[2] = '<span class="form-error" icon="warning">'.dg__e('Please enter an email adress!').'</span>';
			}		
		}
		if(empty($postcontent['contact_phone'])) {
			$validate = FALSE;
			$errormsg[3] = '<span class="form-error" icon="warning">'.dg__e('Please enter your phone number').'</span>';		
		}
		if(empty($postcontent['contact_company'])) {
			$validate = FALSE;
			$errormsg[4] = '<span class="form-error" icon="warning">'.dg__e('Please enter your company name').'</span>';		
		}
		
		if($validate) {
				$recipient = get_option('dgf_send_to_email');
				...
				$str="<font color='#333366'><h2>SATR Digital Forecast Download</h2></font>";
				...
				//$budget = dg_calculate($postcontent['contact_budget']);
				...
				// notification
				$notified_future_digital_subscribe 	= $_POST['notified'];
				if($notified_future_digital_subscribe) 
				{
					$str=$str."<b>I would like to be notified when future Digital Forecasts become available.</b><br/>";
				...
				$str=$str."<b>Time on Site : </b>".dg_convertReadable($_COOKIE['mystaytime'])."<br/>";
				...
				//$headers .= "From: ".$contact_fname.' '.$contact_lname." <".$contact_email.">\r\n";
				$headers .= "Content-Type: text/html; charset=\"".get_settings('blog_charset')."\"\r\n";
				//$fullmsg = dg__e('Sender').': '.$contact_fname.' '.$contact_lname." <".$contact_email.">\n";
				//$fullmsg .= dg__e('Subject').": $subject ...\n";
				$fullmsg .= wordwrap($str, 80, "\n")."\n\n";
				$fullmsg .= "IP: ".$_SERVER['REMOTE_ADDR'];
				//Remove any attempt to modify headers etc
				$fullmsg 				= dg_message_remove_malicious($fullmsg);
				$mail_success 			= 	wp_mail($recipient, get_option('dgf_mail_subject'), $fullmsg, $headers);
				...
				$interest_group 		=	$groupName['digital_forecast'];
			...
			//$contact_referrer = unserialize(base64_decode($_COOKIE['contact_referrer'])); // defined at top
			$data['referral'] = $contact_referrer['search_engine'];
			$data['keywords'] = $contact_referrer['keywords'];
			$data['last_visited'] = $contact_referrer['last_visited'];
			$data['visited'] = $contact_referrer['visited'];
			$data['stay_time'] = $_COOKIE['mystaytime'];
			$data['ip'] = $_SERVER['REMOTE_ADDR'];
			/*$iplocation = @simplexml_load_file('http://ipinfodb.com/ip_query.php?ip='.$_SERVER['REMOTE_ADDR'].'&timezone=false');
			if($iplocation) {
				foreach($iplocation as )	
			}
			...
			*/
			...
			if($mail_success){
					//$output .= '<p icon="accept">'.dg__e('Thank you for taking an interest in our services! We will respond to your inquiry within 24 hours.').'</p>';
					//header('Location: '.$page_url.'?msg='.urlencode('Your message was sent successfully!'));
					$page_next=get_page_by_title('Digital Forecast Thank You');
					$page_url_thanks =  get_permalink($page_next->ID);
					//setcookie('thanks-fname',$postcontent['contact_fname']);
					//echo 'cookie value : '.$postcontent['contact_fname'];
					//setcookie("thanks-fname",$postcontent['contact_fname']); 
					//setcookie('contact_referrer', serialize(array('search_engine' => $search_engine, 'keywords' => $keywords)));
					//print_r ($_COOKIE);
					//setcookie('thankhhh-fname', $postcontent['contact_fname'], time() - 31536000, COOKIEPATH);
					//print_r ($_COOKIE);
					//setcookie("sevisitor", 1, time()+3600, "/", str_replace('http://www','',get_bloginfo('url')) );
					//die('digital forecast');
					//header('Location: '.$page_url_thanks);
					//exit();
					echo "<script type='text/javascript'> window.location.href='".$page_url_thanks ."'</script>";
					exit();
				} else {
					$output .= '<p icon="accept">'.dg__e('There was a problem sending your message, please try again!').'</p>';
				}
    ...
    $output .= '<table width="500" border="0" cellpadding="4" cellspacing="0" class="contactus">
          ...
          <table width="500" border="0" cellspacing="0" cellpadding="4" class= "contactus">
            <tr>
              <td colspan="2">&nbsp;</td>
            </tr>
			...
			<tr>
                <td align="left"><span style="font-weight: bold"> I would like to be notified when future Digital Forecasts become available. </span></td>
                <td style="text-align:left"><input name="notified" type="checkbox" id="notified"      value="1" checked="checked" style="border:none;" /></td>
            </tr>
          </table>';
	...
	*/
    ...
    $output .= '<table width="480" border="0" cellspacing="0" cellpadding="10">
            <tr>
              <td width="240"><center>
              ...
              <td width="220" align="center"><input type="submit" name="dg_submit" value="Submit" /></td>
            </tr>
          </table>';
...
function dg_scf_add_admin_page() {
	add_options_page('Contact Digital Forecast options', 'Digital Forecast Form options', 8, __FILE__, 'dg_contact_digital_forecast_form_admin_page');
}

function dg_contact_digital_forecast_form_admin_page() { 
	...
	if(isset($_POST[dg_submit])) {
		...
		update_option('dgf_send_to_email', $postcontent[send_to_email]);
		update_option('scf_captcha_num_letters', $postcontent[captcha_num_letters]);
		update_option('dgf_mail_subject', $postcontent[dgf_mail_subject]);
	...
	<?php if(!empty($ui_feedback)) echo '<div id="message" class="updated fade"><p>'.$ui_feedback.'</p></div>'; ?>
		<h2>Contact Digital Forecast</h2>
			<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
				<fieldset class="options">
					 ...
					 <tr valign="top">
						...
						<td align="left">
							<input type="text" name="send_to_email" value="<?php echo get_option('dgf_send_to_email'); ?>" size="40" />
						</td>
					</tr>
					...
					<tr valign="top">
						...
						<td align="left">
							<input type="text" name="dgf_mail_subject" value="<?php echo get_option('dgf_mail_subject'); ?>" size="40" />
						</td>
					</tr>
				...
				<input type="submit" name="dg_submit" value="Submit" />
				</fieldset>
			</form>
	<h2>Usage</h2>
	To use Contact Digital Forecast Form, simply add a page with the following text:
	<code>[dg_simple_contact_form]</code>
	<h2>Localization</h2>
...
add_action('admin_menu', 'dg_scf_add_admin_page');

add_filter('the_content', 'place_dg_contact_digital_forecast_form', '7');

function place_dg_contact_digital_forecast_form($content){
	 $output = dg_contact_digital_forecast_form();
     $content = str_replace( '[dg_simple_contact_form]', $output, $content); 
	 return $content;
}

function dg_writeCSS() {
...
add_action('wp_head', 'dg_writeCSS');
