		...
		*/
		...
		switch($mail_bank_version_number)
		{
			...
			if(count($wpdb->get_var("SHOW TABLES LIKE '" . mail_bank() . "'")) != 0 && count($wpdb->get_var("SHOW TABLES LIKE '" . mail_bank_meta() . "'")) != 0)
			{
				...
				if(!array_key_exists("other_roles_privileges", $get_roles_settings_data_array))
				{
					$get_roles_settings_data_array["other_roles_privileges"] = "0,0,0,0,0,0,0,0";
					$roles_capabilities_data  = explode("," ,$get_roles_settings_data_array["roles_and_capabilities"]);
					if(count($roles_capabilities_data) == 5)
					{
						array_push($roles_capabilities_data,0);
						$get_roles_settings_data_array["roles_and_capabilities"] = implode(",",$roles_capabilities_data);
					}
					$where = array();
		...
		update_option("mail-bank-version-number","2.0.2");
	}
}
?>
