...
/**
* @see mail_bank_Zend_Mail_Storage_Folder_Maildir
*/
if(file_exists(MAIL_BANK_DIR_PATH.'lib/zend/mail/storage/folder/maildir.php'))
require_once MAIL_BANK_DIR_PATH.'lib/zend/mail/storage/folder/maildir.php';

/**
* @see mail_bank_Zend_Mail_Storage_Writable_Interface
*/
if(file_exists(MAIL_BANK_DIR_PATH.'lib/zend/mail/storage/writable/interface.php'))
require_once MAIL_BANK_DIR_PATH.'lib/zend/mail/storage/writable/interface.php';
...
* @license http://framework.zend.com/license/new-bsd  New BSD License
*/
class mail_bank_Zend_Mail_Storage_Writable_Maildir extends mail_bank_Zend_Mail_Storage_Folder_Maildir implements mail_bank_Zend_Mail_Storage_Writable_Interface
{
	...
	* @return null
	* @throws mail_bank_Zend_Mail_Storage_Exception
	*/
	...
	* @param string|mail_bank_Zend_Mail_Storage_Folder $parentFolder parent folder for new folder, else root folder is parent
	...
	*/
	public function createFolder($name, $parentFolder = null)
	{
		...
		$this->getFolders($parent)->$localName = new mail_bank_Zend_Mail_Storage_Folder($localName, $folder, true);
	...
	* remove a folder
	*
	* @param	string|mail_bank_Zend_Mail_Storage_Folder $name		name or instance of folder
	* @return  null
	* @throws  mail_bank_Zend_Mail_Storage_Exception
	*/
	...
	*
	* @param	string|mail_bank_Zend_Mail_Storage_Folder $oldName name or instance of folder
	...
	protected function _createTmpFile($folder = 'INBOX')
	{
			...
			if (!file_exists($tmpdir . $uniq))
			{
				...
				if (!$fh)
				{
					...
					*/
					...
					throw new mail_bank_Zend_Mail_Storage_Exception('could not open temp file');
				}
				break;
			}
			sleep(1);
		...
		if (!$fh)
		{
			...
			*/
			...
			throw new mail_bank_Zend_Mail_Storage_Exception("tried $max_tries unique ids for a temp file, but all were taken". ' - giving up');
		}
	...
	*/
	protected function _getInfoString(&$flags)
	{
		...
		if (isset($wanted_flags[mail_bank_Zend_Mail_Storage::FLAG_RECENT]))
		{
			/**
			* @see mail_bank_Zend_Mail_Storage_Exception
			*/
			...
			throw new mail_bank_Zend_Mail_Storage_Exception('recent flag may not be set');
		}

		$info = ':2,';
		$flags = array();
		foreach (mail_bank_Zend_Mail_Storage_Maildir::$_knownFlags as $char => $flag)
		{
		...
		if (!empty($wanted_flags))
		{
			...
			*/
			...
			throw new mail_bank_Zend_Mail_Storage_Exception('unknown flag(s): ' . $wanted_flags);
		}
	...
	* @param null|string|mail_bank_Zend_Mail_Storage_Folder $folder  folder for new message, else current folder is taken
	* @param null|array $flags	set flags for new message, else a default set is used
	* @param bool $recent  handle this mail as if recent flag has been set,
	* should only be used in delivery
	* @throws  mail_bank_Zend_Mail_Storage_Exception
	*/
	// not yet * @param string|mail_bank_Zend_Mail_Message|mail_bank_Zend_Mime_Message $message message as string or instance of message class
	...
	public function appendMessage($message, $folder = null, $flags = null, $recent = false)
	{
		if ($this->_quota && $this->checkQuota())
		{
			...
			*/
			...
			throw new mail_bank_Zend_Mail_Storage_Exception('storage is over quota!');
		}
		...
		if (!($folder instanceof mail_bank_Zend_Mail_Storage_Folder))
		{
		...
		if ($flags === null)
		{
			$flags = array(mail_bank_Zend_Mail_Storage::FLAG_SEEN);
		}
		...
		if (!link($temp_file['filename'], $new_filename))
		{
			...
			if(file_exists(MAIL_BANK_DIR_PATH.'lib/zend/mail/storage/exception.php'))
			require_once MAIL_BANK_DIR_PATH.'lib/zend/mail/storage/exception.php';
			$exception = new mail_bank_Zend_Mail_Storage_Exception('cannot link message file to final dir');
		}
	...
	* @param string|mail_bank_Zend_Mail_Storage_Folder $folder name or instance of targer folder
	* @return  null
	* @throws  mail_bank_Zend_Mail_Storage_Exception
	*/
	public function copyMessage($id, $folder)
	{
		...
		while (($key = array_search(mail_bank_Zend_Mail_Storage::FLAG_RECENT, $flags)) !== false)
		{
			unset($flags[$key]);
		}
		$info = $this->_getInfoString($flags);
		...
		if (!copy($old_file, $temp_file['filename']))
		{
			...
			*/
			...
			$exception = new mail_bank_Zend_Mail_Storage_Exception('cannot copy message file');
		}
	...
	public function moveMessage($id, $folder)
	{
			...
			|| ($this->_currentFolder == 'INBOX' && $folder->getGlobalName() == '/'))
		{
			...
			*/
			...
			throw new mail_bank_Zend_Mail_Storage_Exception('target is current folder');
		}
		...
		if (!rename($old_file, $new_file))
		{
			...
			*/
			...
			$exception = new mail_bank_Zend_Mail_Storage_Exception('cannot move message file');
		}
		@unlink($temp_file['filename']);
	...
	public function setFlags($id, $flags)
	{
		...
		if (!@rename($filedata['filename'], $new_filename))
		{
			...
			*/
			...
			throw new mail_bank_Zend_Mail_Storage_Exception('cannot rename file');
		}
	...
	public function removeMessage($id)
	{
		...
		if (!@unlink($filename))
		{
			...
			*/
			...
			throw new mail_bank_Zend_Mail_Storage_Exception('cannot remove message');
		}
		unset($this->_files[$id - 1]);
		// remove the gap
		$this->_files = array_values($this->_files);
	...
	*
	* @see mail_bank_Zend_Mail_Storage_Writable_Maildir::setQuota()
	*
	* @return bool|array
	*/
		...
		if ($fromStorage)
		{
			...
			if (!$fh)
			{
				...
				*/
				...
				throw new mail_bank_Zend_Mail_Storage_Exception('cannot open maildirsize');
			}
			$definition = fgets($fh);
			fclose($fh);
	...
	*/
	protected function _calculateMaildirsize()
	{
		...
		else
		{
			...
			{
				throw new mail_bank_Zend_Mail_Storage_Exception('no quota definition found', 0, $e);
			}
		}
